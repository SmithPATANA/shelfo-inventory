-- Enhanced Sales Functionality Migration
-- This migration adds additional features and improvements to the sales system

-- Add a constraint to ensure total_amount is calculated correctly
ALTER TABLE sales ADD CONSTRAINT sales_total_amount_check 
CHECK (total_amount >= 0 AND total_amount = (
  SELECT p.selling_price * s.quantity 
  FROM products p 
  WHERE p.id = s.product_id
));

-- Create a function to automatically calculate total_amount
...
-- Create trigger to automatically calculate total_amount before insert
CREATE TRIGGER calculate_sale_total_trigger
  BEFORE INSERT ON sales
  FOR EACH ROW
  EXECUTE FUNCTION calculate_sale_total();

-- Create trigger to automatically calculate total_amount before update
...
-- Create trigger to automatically update product quantity after sale
CREATE TRIGGER update_product_quantity_trigger
  AFTER INSERT ON sales
  FOR EACH ROW
  EXECUTE FUNCTION update_product_quantity_after_sale();

-- Create a function to validate sale quantity
...
-- Create trigger to validate sale quantity before insert
...
-- Create a function to get sales summary for a user
